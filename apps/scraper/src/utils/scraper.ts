...
/**
 * Search for courses using a pre-established session.
 * This reuses the ICSID and cookies from an existing session.
 */
export async function searchWithSession(
  ...
  const { fetch: fetchFn, icsid } = createFetchWithSession(session);
...
/**
 * Handle scraping using a pre-established session.
 * Handles the 300-result limit by splitting into language queries.
 */
export async function handleScrapingWithSession(
  session: SessionData,
  term: Term,
  year: number,
  subject: string,
  ...
  const html = await searchWithSession(
    session,
    term,
    year,
    subject,
